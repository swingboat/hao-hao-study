 ...
 * practice_item staging（一行一题）+ 进度模型用 ItemProgressSnapshot。
 */
...
import { ITEM_PROMPT_VERSION, type ItemProgressSnapshot, runItemAnalysis } from './item-pipeline';
...
async function patchProgress(jobId: string, snap: ItemProgressSnapshot): Promise<void> {
 ...
 * 不抛错。
 */
export async function runItemParse(
  jobId: string,
  uploadId: string,
  providerId: string,
  subjectId: string,
  ...
  try {
      ...
      data: {
        ...
        raw_response: {
          progress: {
            ...
            lastEvent: 'started',
          } satisfies ItemProgressSnapshot,
    ...
    const tmpDir = path.join(tmpdir(), 'hao-admin-item-parse');
    ...
    if (kpDictionary.length > 0) {
      await prisma.llm_parse_job.update({
        where: { id: jobId },
        data: { prompt_version: `${ITEM_PROMPT_VERSION}+kpdict-${kpDictionary.length}` },
      });
    }

    const result = await runItemAnalysis({
      jobId,
      providerId,
      pdfPath: tmpPath,
      ...
      onProgress: (snap) => {
        void patchProgress(jobId, snap).catch((e) =>
          console.warn(`[item-pipeline job=${jobId}] progress patch fail:`, e),
        );
      },
    });
    ...
    await prisma.$transaction([
      prisma.llm_parse_staging.createMany({
        data: result.items.map((it) => ({
          parse_job_id: jobId,
          upload_id: uploadId,
          entity_kind: 'practice_item' as const,
          llm_payload: {
            content: it.content,
            ...
            // _src_page / item_no 来自 extractItemsFromPdf 的内部字段（默认 chunk prompt 强制 LLM 自报）
            source_hint: {
              page: it._src_page ?? null,
      ...
      prisma.llm_parse_job.update({
        ...
        data: {
          ...
          request_payload: redactAuthHeaders({
            note: 'L2 extractItemsFromPdf；representative payload 见 packages/llm vision 层日志',
          }) as Prisma.InputJsonValue,
          raw_response: {
    ...
    console.error(`[item runParse job=${jobId}] FAILED:`, msg);
    try {
      await prisma.llm_parse_job.update({
        where: { id: jobId },
        data: {
      ...
      console.error(`[item runParse job=${jobId}] failed-patch error:`, patchErr);
    }
  } finally {
    if (tmpPath) {
