# admin端 MVP PRD — AI Native Operator Console v0.1

> 本文件为admin端独立 PRD，与 `SmartLearningAssistant_PRD.md` v1.0 并列。web端能力请参见主 PRD。
>
> | 版本 | 日期 | 说明 |
> |---|---|---|
...
> **admin端是一个 AI 优先的内容工厂控制台：让单个admin人员在不写 SQL 的前提下，通过"上传文件 → LLM 抽取 → 人工 diff 确认"的工作流，30 分钟内将一份教材或题集转化为可被web端消费的 KP 与题目，并完成学生开户与合规工单处理。**

| 维度 | 定义 |
|---|---|
| 核心解决目标 | 把admin冷启动从"手敲 200 题 + 手填 50 KP"的 2 周工作量，压缩到"上传 + 审核"的 1–2 天 |
| 核心用户 | 内部admin / 教研人员（v0.1 阶段单一管理员账号，无角色划分） |
| 核心价值主张 | LLM 干脏活，人只做"接受 / 拒绝 / 微调"判断 |
| 部署形态 | Web 后台（与web端同域不同路径，如 `/admin`） |
...
**F1.1 管理员登录**
- **当** admin访问 `/admin` 任意页面但未持有有效 session **时**，
...
- ❌ 自定义 prompt 编辑器。Prompt 由代码内常量维护，admin仅可在下拉中选 `prompt_version`。
...
### §3.8 通用
- ❌ 多语言 i18n。v0.1 仅简体中文。
- ❌ 移动端适配。admin端仅保证桌面 Chrome ≥ 120 可用。
- ❌ 暗色模式。
- ❌ 操作撤销 / 历史版本回滚。
...
7. 在 `/admin/students` 通过 F5.1 确认 seed 学生 `niki / Niki` 可见。第一轮 MVP 到此完成admin端内容冷启动和学生列表展示闭环；学生开户、学生登录首个 Session 与看板展示进入后续轮次。
...
| 编号 | 指标 | 阈值 | 测量方式 |
|---|---|---|---|
| Q1 | 题目解析admin接受率（未做任何字段修改即通过的比例） | ≥ 70% | 统计 staging.review_status=`accepted` 且 `review_payload == llm_payload` 的占比 |
| Q2 | 题目解析端到端延迟（上传完成 → staging 列表可见，单文件 ≤ 50 题） | P95 ≤ 60s | llm_parse_job.latency_ms 分布 |
| Q3 | 单题 diff 平均处理时长（打开抽屉 → 接受/丢弃） | ≤ 30s | 前端埋点 |
| Q4 | F2.2 切换 Provider 重跑后，admin改判率（"重跑后被采纳" 而 "原结果被丢弃" 的比例） | ≥ 30% 视为该 Provider 切换有效 | llm_parse_job 配对统计 |
...
> 完整字段表见主 PRD §3 / §10.4。本节仅列出admin端新增 4 张表。
...
| §2 F2 / F3.6 / F7 | 本文件新增（主 PRD 无admin端 LLM 解析章节） |
| §3 排他边界 | §10.3 砍掉的模块清单 + §8.2 延后决议 + §9 待澄清 |
| §6 数据模型 | §3.3 + §10.4（本文件表 11–14 为admin端扩展） |
