...
| practice_item | 题目（500-1000 道/数学），状态变更审计统一走 `audit_log`（target_type=`practice_item`） |
...
> 题目状态变更审计日志已统一并入通用 `audit_log`（详见本节后部 §3.3 末尾）。题目状态流转通过 `audit_log` 落盘：`target_type='practice_item'`，`action='item.status_change'`，`payload` 内携带 `from_status / to_status / reason` 等历史字段。
...
| action | string | ✓ | 受控集合 | `consent_confirmed` / `export_data_self` / `export_data` / `delete_self` / `delete_student` / `consent_withdraw`(v0.2+) / `item.status_change` |
| target_type | string | ✓ | 目标对象类型 | `student` / `practice_item` / ... |
...
**保留策略**：永不硬删。关联 `practice_item.status=deprecated` 时仍保留日志。

**典型 action 与 payload 约定**：

| 场景 | action | target_type | payload 关键字段 |
|---|---|---|---|
| 题目状态流转（draft→pending→...→deprecated） | `item.status_change` | `practice_item` | `from_status`, `to_status`, `reason` |
...
audit_log
  id          uuid PK
  actor_id    uuid
  action      text    -- "delete_student" | "export_data" | "consent_withdraw" | "item.status_change" 等
  target_type text    -- "student" | "practice_item" | ...
  target_id   uuid
