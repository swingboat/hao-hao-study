...
| 层 | 选型 | 用途 |
|---|---|---|
| 抽象层 | **自写 `callLLM(providerId, prompt, schema?)`**（packages/llm） | 统一 OpenAI 协议 + Google generateContent 协议 |
| Provider 1 | `webex-gemini-3.1-pro`（OpenAI 协议） | 文本/PDF 解析（默认 KP / Goal Template 解析） |
