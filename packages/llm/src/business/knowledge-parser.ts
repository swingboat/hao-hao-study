...
export async function parsePdfKnowledgePoints({
  parsePdfPagesImpl = parsePdfPages,
  ...options
}) {
  const documentResult = await parsePdfPagesImpl({
    ...options,
    pagePrompt: options.pagePrompt ?? (({ pageNumber, totalPages }) => buildKnowledgePagePrompt({ pageNumber, totalPages })),
    finalPrompt: options.finalPrompt ?? (({ pageResults }) => buildKnowledgeFinalPrompt({ pageResults }))
  });
  const parsed = parseKnowledgePointsJson(documentResult.text);
  const fallback = parsed.knowledge_points.length > 0
    ? null
  ...
  return value
    ...
    .map((chapter) => {
      ...
      return omitUndefined({
        ...
        title: heading.title,
        display_name: chapter.display_name == null && chapter.displayName == null
          ? heading.displayName
  ...
  return value
    ...
    .map((section) => {
      ...
      return omitUndefined({
        number: heading.number,
        title: heading.title,
        display_name: section.display_name == null && section.displayName == null
          ? heading.displayName
...
function normalizeKnowledgePoints(value, {
  ...
  return value
    ...
    .map((point) => omitUndefined({
      chapter_number: point.chapter_number == null && point.chapterNumber == null
        ? chapterNumber
        : String(point.chapter_number ?? point.chapterNumber),
      chapter_title: point.chapter_title == null && point.chapterTitle == null
        ? chapterTitle
        : String(point.chapter_title ?? point.chapterTitle),
      section_number: point.section_number == null && point.sectionNumber == null
        ? sectionNumber
        : String(point.section_number ?? point.sectionNumber),
      section_title: point.section_title == null && point.sectionTitle == null
        ? sectionTitle
...
function mergeKnowledgePoints(points) {
  ...
  for (const point of points) {
    ...
    merged.set(key, {
      ...
      prerequisites: uniqueStrings([...(existing.prerequisites ?? []), ...(point.prerequisites ?? [])]),
      source_pages: uniqueNumbers([...(existing.source_pages ?? []), ...(point.source_pages ?? [])]),
      raw: existing.raw
    });
  }
...
function normalizeCoverageSummary(value, { outputKnowledgePointCount }) {
  ...
  return {
    input_candidate_count: numberOrDefault(summary.input_candidate_count ?? summary.inputCandidateCount, null),
    output_knowledge_point_count: outputKnowledgePointCount,
    expected_range: summary.expected_range == null && summary.expectedRange == null
...
function parseKnowledgePointsFromPageResults(pageResults) {
  ...
  for (const page of pageResults) {
    ...
    const sectionEntry = sectionTitle || sectionNumber
  ...
  if (!chapters.some((chapter) => chapter.knowledge_points.length > 0 || chapter.sections.some((section) => section.knowledge_points.length > 0))) {
    return null;
  }
  ...
  return parseKnowledgePointsJson(JSON.stringify({
    coverage_summary: {
      input_candidate_count: inputCandidateCount,
      output_knowledge_point_count: 0,
      expected_range: DEFAULT_TEXTBOOK_KNOWLEDGE_POINT_TARGET_RANGE,
