...
export async function parseQuestionPages({
  ...
  const documentResult = await parseDocumentPagesImpl({
    ...
    pagePrompt:
      options.pagePrompt ??
      (({ pageNumber, totalPages }) => buildQuestionPagePrompt({ pageNumber, totalPages })),
    finalPrompt:
      options.finalPrompt ?? (({ pageResults }) => buildQuestionFinalPrompt({ pageResults })),
  });
...
export async function parseWordQuestions({ parseWordPagesImpl = parseWordPages, ...options }) {
...
export async function parsePdfQuestions({ parsePdfPagesImpl = parsePdfPages, ...options }) {
...
function isLikelyQuestion(question) {
  ...
  if (hasNonQuestionInstructionSignal(question) && !hasActionableQuestionSignal(question))
    return false;
  return true;
}
...
function questionSearchText(question) {
  ...
  for (const subQuestion of question.sub_questions ?? []) {
    parts.push(subQuestion.number, subQuestion.stem, subQuestion.answer, subQuestion.analysis);
  }
...
function questionActionSearchText(question) {
  ...
  for (const subQuestion of question.sub_questions ?? []) {
    parts.push(subQuestion.stem, subQuestion.answer, subQuestion.analysis);
  }
  return parts
    .filter(Boolean)
    .join('\n')
...
const ACTIONABLE_QUESTION_PATTERN =
  /(?<!要)求|计算|证明|选择|选出|填空|填写|回答|解答|完成|补全|写出|列出|判断|化简|解方程|作图|画出|说明理由|请说明|问[:：]|多少|哪(?:个|项|一项)|下列|正确|错误|_{2,}|____|（\s*）|\(\s*\)|\?|？/;
const NON_QUESTION_INSTRUCTION_PATTERN =
  /题型\s*[一二三四五六七八九十\d]+|题型讲解|知识点|方法总结|考点|技巧|口诀|扫码|二维码|公众号|课程|视频|讲解|例题|广告|LOOK AT ME|看成一个整体|插空法|捆绑法/i;
...
function normalizeAnalysis(question) {
  const value =
    question.analysis ?? question.explanation ?? question.solution ?? question.answer_analysis;
...
function normalizeQuestionLike(question, { defaultType, includeRaw } = {}) {
  const normalized = omitUndefined({
    ...
    figure_description:
      question.figure_description == null ? undefined : String(question.figure_description),
  ...
  const tables = value
    .map((table) => {
      ...
      const normalized = omitUndefined({
        ...
        owner_path:
          table.owner_path == null && table.ownerPath == null
            ? undefined
            : String(table.owner_path ?? table.ownerPath),
        source_page: numberOrUndefined(
          table.source_page ?? table.sourcePage ?? table.page_number ?? table.page,
        ),
        title:
          table.title == null && table.caption == null
            ? undefined
            : String(table.title ?? table.caption),
  ...
  const figures = value
    .map((figure) => {
      ...
      return omitUndefined({
        ...
        group_id:
          figure.group_id == null && figure.groupId == null
            ? undefined
            : String(figure.group_id ?? figure.groupId),
        role: normalizeRole(figure.role ?? figure.kind ?? figure.placement),
        owner_path:
          figure.owner_path == null && figure.ownerPath == null
            ? undefined
            : String(figure.owner_path ?? figure.ownerPath),
        source_page: numberOrUndefined(
        ...
        subfigures: normalizeSubfigures(
          figure.subfigures ?? figure.sub_figures ?? figure.subFigures,
        ),
