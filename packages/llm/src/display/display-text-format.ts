...
export function formatDisplayText(value) {
  return formatLatexCommands(String(value ?? "")
    .replace(/\r\n/g, "\n")
...
function formatLatexCommands(value) {
  return formatCardinalityExpressions(String(value ?? "")
    .replace(/\\\\(?=[A-Za-z])/g, "\\")
    .replace(/≠g\b/g, "¬")
    ...
    .replace(/\\left\s*([()\[\]{}|.])/g, "$1")
    .replace(/\\right\s*([()\[\]{}|.])/g, "$1")
    .replace(/\\(?:dfrac|tfrac|frac)\{([^{}]+)\}\{\\sqrt\[([^\]]+)\]\{([^{}]+)\}\}/g, (_, numerator, index, radicand) => formatFraction(numerator, `${toSuperscript(index)}√${radicand}`))
    .replace(/\\(?:dfrac|tfrac|frac)\{([^{}]+)\}\{([^{}]+)\}/g, (_, numerator, denominator) => formatFraction(numerator, denominator))
    ...
    .replace(/\\sqrt\[([^\]]+)\]\{([^{}]+)\}/g, (_, index, radicand) => `${toSuperscript(index)}√${radicand}`)
    .replace(/\\sqrt\[([^\]]+)\]([A-Za-z0-9]+)/g, (_, index, radicand) => `${toSuperscript(index)}√${radicand}`)
    .replace(/\\sqrt\{([^{}]+)\}/g, "√$1")
    ...
    .replace(/\\%/g, "%")
    .replace(/\\pi/g, "π")
    .replace(/([A-Za-z0-9)ℕℤℚℝℂ])\^\(([0-9A-Za-z+*\/-]+)\)/g, (_, base, exponent) => base + toSuperscript(exponent))
    .replace(/([A-Za-z0-9)ℕℤℚℝℂ])\^\{([0-9A-Za-z+*\/-]+)\}/g, (_, base, exponent) => base + toSuperscript(exponent))
    .replace(/([A-Za-z0-9)ℕℤℚℝℂ])\^([0-9*])/g, (_, base, exponent) => base + toSuperscript(exponent))
    .replace(/([A-Za-z0-9)ℕℤℚℝℂ])\^([A-Za-z])/g, (_, base, exponent) => base + toSuperscript(exponent))
    .replace(/([A-Za-z])_\{([0-9A-Za-z]+)\}/g, (_, base, subscript) => base + toSubscript(subscript))
