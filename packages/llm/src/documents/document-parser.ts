...
export async function parseDocumentPages({
  ...
  const pageResults = await mapConcurrent(normalizedPages, normalizeConcurrency(concurrency), async (page) => {
    onProgress?.({
      stage: "page_started",
    ...
    const response = await callPageWithRetries({
      ...
      operation: () => callLlmWithCache({
        ...
        onCacheHit: () => emitCacheHit(onProgress, {
...
export async function renderPdfToPageImages({ name, data, path: inputPath, outputDir, dpi = 180 } = {}) {
  const workDir = outputDir ? path.resolve(outputDir) : await mkdtemp(path.join(os.tmpdir(), "llm-proxy-pdf-pages-"));
  await mkdir(workDir, { recursive: true });
  const shouldCleanup = !outputDir;
  ...
  try {
    ...
    const pages = await Promise.all(files.map(async (file, index) => omitUndefined({
      pageNumber: index + 1,
      mimeType: "image/png",
...
function resultFromPageResponses({ documentType, target, pageResults, finalResponse }) {
  ...
  const pageText = pageResults.map((page) => page.text).filter(Boolean).join("\n\n");
  ...
  return omitUndefined({
    ...
    ok: finalResponse ? finalResponse.ok && pageResults.every((page) => page.ok) : pageResults.every((page) => page.ok),
...
function imageRefsFromPages(pages) {
  return pages
    .map((page) => page.source_image_ref ?? pageImageRefFromPage(page))
    .filter(Boolean);
}
...
function isDocumentCache(cache) {
  return cache
    && typeof cache.getJson === "function"
    && typeof cache.setJson === "function";
}
...
function llmOptionFingerprint(request) {
  const ignored = new Set(["llmConfig", "llmTarget", "targetConfig", "target", "input", "attachments", "maxTokens"]);
...
function resolvePagePrompt({ pagePrompt, documentType, page, totalPages }) {
  ...
  return pagePrompt ?? buildPagePrompt({
    documentType,
    pageNumber: page.pageNumber
  });
}
...
function resolveFinalPrompt({ finalPrompt, documentType, pageResults }) {
  ...
  return finalPrompt ?? buildFinalDocumentPrompt({
    documentType,
    pageResults
  });
}
...
function sumUsageField(usages, field) {
  const values = usages.map((usage) => numberOrNull(usage?.[field])).filter((value) => value != null);
