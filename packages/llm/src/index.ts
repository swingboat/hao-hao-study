 ...
 *   - providers/bedrock-converse.ts         AWS Bedrock Converse 协议（原生 PDF）
 *   - providers/types.ts                    ProviderAdapter 接口 + Attachment 类型
 *   - pdf/qpdf.ts                           qpdf 命令封装（页数 + 切片）
 *   - pdf/analyze-pdf.ts                    PDF 分片 + 循环 callLLM + 终审整合
