...
export function extractLlmText(body, apiShape) {
  ...
  if (apiShape === 'openai-chat-completions' || apiShape === 'azure-chat-completions') {
    ...
    return joinText(
      body.choices.map((choice) => textFromContent(choice.message?.content ?? choice.text)),
    );
  }
  ...
  if (apiShape === 'openai-responses') {
    ...
    return joinText(
      body.output.flatMap((item) => textFromContent(item.content ?? item.text ?? item)),
    );
  }
  ...
  if (apiShape === 'google-generate-content') {
    ...
    return joinText(
      body.candidates.flatMap(
        (candidate) => candidate.content?.parts?.map((part) => part.text) ?? [],
      ),
    );
  }
...
function buildUrl(baseUrl, target) {
  ...
  const url = /^https?:\/\//i.test(target.path)
    ? new URL(target.path)
...
function sanitizeBedrockDocumentName(name) {
  const sanitized = String(name ?? 'document')
    .replace(/[^a-zA-Z0-9-]/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '');
