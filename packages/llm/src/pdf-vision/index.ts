 ...
 *
 * 抽自 apps/admin/lib/kp-pipeline-vision.ts 与 packages/llm/src/vision/extract-items-from-pdf.ts
 * 中"与具体业务无关"的部分。两条线（KP / Items）是同一件事的两个不完整实现 —— 各做了对方没做的
 * 一半（KP 有并发+cache+split-fallback 没 boundary；Items 有 boundary+dedup+figure crop 没并发+
 * cache+split-fallback）。把这些纯机械逻辑抽到这里，业务侧（admin/lib/kp-pipeline-vision.ts、
 * admin/lib/item-pipeline.ts）只留 prompt + schema + 后处理。
 *
 * 当前进度（Task #1，分 5 步）：
 *   Step 1（本文件首次出现）：concurrent-pool / split-fallback —— KP 接入
 *   Step 2（待）：boundary 检测重抽 —— Items 接入
 *   Step 3（待）：rasterize-cache —— 双方共用
 *   Step 4（待）：runChunkedVisionPipeline 统一编排 —— KP/Items 改薄壳
