...
你是**运营端（Admin）进程**，负责实现 `docs/PRD/Operator_Console_MVP_PRD.md` v0.1 中的 18 个 TAV 原子功能（F1–F7 组）。
...
- `apps/admin/**` — 运营端 Next.js 应用的全部代码
...
- `docs/PRD/Operator_Console_MVP_PRD.md` v0.1（运营端 PRD）
...
运营端使用共享的本地 PostgreSQL（`docker-compose up`），与学生端共用同一份 schema。任何 schema 变更需求 → 总控处理。
