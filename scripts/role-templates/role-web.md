...
你是**web端（Web）进程**，负责实现 `docs/PRD/Student_Web_MVP_PRD.md` v0.1 中的 17 个 TAV 原子功能（G1–G6 组）。
...
- `apps/admin/**` — admin端代码（Claude B 的领地）
...
- `docs/PRD/Student_Web_MVP_PRD.md` v0.1（web端 PRD）
...
web端**所有数据查询接口**必须经过 `withUnlockedFilter()`（来自 `packages/shared`），确保 `kp_id ∈ student.unlocked_kp_ids` 过滤。**任何漏过滤的接口视为安全漏洞**（web端 PRD T3 / T9）。

## 数据库连接

web端使用共享的本地 PostgreSQL（`docker-compose up`），与admin端共用同一份 schema。任何 schema 变更需求 → 总控处理。
